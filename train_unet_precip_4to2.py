...
class Config:
    ...
    save_dir = "/mnt/hdd1/wubo/projects/yaan_precip_4to2/checkpoints_4to2"
...
@torch.no_grad()
def validate_one_epoch(model, loader, criterion, device, use_log1p=True, use_mask_loss=False, csi_threshold=0.1):
    model.eval()
    total_loss = 0.0
    total_mae = 0.0
    total_rmse = 0.0
    ...
    for x, y, target_mask in loader:
        ...
        if use_mask_loss:
            loss = criterion(pred, y, target_mask)
            mae, rmse = calc_metrics(pred, y, use_log1p, target_mask)
            csi = calc_csi(pred, y, threshold=csi_threshold, use_log1p=use_log1p, mask=target_mask)
        else:
            loss = criterion(pred, y, None)
            mae, rmse = calc_metrics(pred, y, use_log1p, None)
            csi = calc_csi(pred, y, threshold=csi_threshold, use_log1p=use_log1p, mask=None)
